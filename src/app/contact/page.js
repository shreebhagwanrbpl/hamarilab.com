...
export default function ContactPage() {
  ...
  useEffect(() => {
    const loadDistrict = async () => {
      ...
      try {
        const snap = await getDoc(
          doc(db, "websites", "hamarilabcom", "districts", currentDistrict)
        );
        if (snap.exists()) {
                ...
                {loading ? (
                  <div className="space-y-4">
                    {[...Array(3)].map((_, i) => (
                      <div
                        key={i}
                        className="h-28 rounded-3xl bg-[#e6f8f3]/60 animate-pulse"
                      />
                    ))}
                  </div>
                ) : contactInfo.length === 0 ? (
                  <div className="rounded-3xl border border-slate-200 bg-white p-8 text-center text-slate-600">
                    <Info size={32} className="mx-auto text-[#0d9488] mb-2" />
                  ...
                  contactInfo.map((item, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className="flex items-start gap-4 rounded-3xl border border-slate-200/90 bg-white p-6 shadow-sm transition-all hover:border-[#0d9488]/40 hover:shadow-md"
                      >
                        <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#e6f8f3] text-[#0d9488] shrink-0">
                          {getFieldIcon(label)}
                        </div>
                              ...
                              if (isPhone) {
                                return (
                                  <a
                                    ...
                                    className="text-base sm:text-lg font-bold text-[#0f172a] hover:text-[#0d9488] transition-colors inline-block"
                                  >
                                    {val}
                                  </a>
                                );
                              ...
                              if (isEmail) {
                                return (
                                  <a
                                    key={vIdx}
                                    href={`mailto:${val}`}
                                    className="text-base font-bold text-[#0f172a] hover:text-[#0d9488] transition-colors inline-block break-all"
                                  >
                                    {val}
                                  </a>
                                );
          ...
          <div className="mt-12 space-y-4">
            {faqs.map((faq, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="rounded-3xl border border-slate-200 bg-[#f0fdf9]/60 overflow-hidden transition-all"
                >
                  <button
                    onClick={() => setOpenFaq(isOpen ? null : idx)}
                    className="w-full flex items-center justify-between p-6 text-left font-bold text-[#0f172a] hover:text-[#0d9488]"
                  >
                    <span className="text-base sm:text-lg pr-4">{faq.q}</span>
                    <ChevronDown
                      size={20}
                      className={`shrink-0 text-[#0d9488] transition-transform duration-300 ${isOpen ? "rotate-180" : ""
                        }`}
                    />
                  </button>

                  {isOpen && (
                    <div className="px-6 pb-6 text-sm sm:text-base leading-relaxed text-slate-600 border-t border-slate-200/50 pt-4">
                      {faq.a}
                    </div>
                  )}
                </div>
